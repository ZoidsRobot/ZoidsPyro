...
def git():
    ...
    try:
        repo = Repo()
        LOGGER("PyroZen").info(f"Git Client Found")
    except GitCommandError:
        LOGGER("PyroZen").info(f"Invalid Git Command")
        ...
        LOGGER("PyroZen").info("Fetched Latest Updates")
    ...
    if is_heroku:
        if HEROKU_API_KEY and HEROKU_APP_NAME:
            try:
                ...
                LOGGER("PyroZen").info(f"Heroku App Configured")
...
async def create_botlog(client):
    ...
    desc = "Group Log untuk PyroZen-UserBot.\n\nHARAP JANGAN KELUAR DARI GROUP INI.\n\n💢 Powered By ~ @Zenc0de 💢"
    try:
        gruplog = await client.create_supergroup("PyroZen UserBot Log", desc)
        if await in_heroku():
