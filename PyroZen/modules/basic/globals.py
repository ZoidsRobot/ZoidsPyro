...
from PyroZen import *
from PyroZen.helpers.adminHelpers import DEVS, WHITELIST
from PyroZen.helpers.basic import edit_or_reply
from PyroZen.helpers.PyroHelpers import get_ub_chats
from PyroZen.utils import extract_user, extract_user_and_reason
...
def globals_init():
    try:
        ...
        sql = import_module("PyroZen.helpers.SQL.gban_sql")
        sql2 = import_module("PyroZen.helpers.SQL.gmute_sql")
...
async def gban_user(client: Client, message: Message):
    ...
    if user_id in DEVS:
        return await Man.edit("**Gak Bisa Di Gban Kontol Zenena dia Yang Buat Aku 🗿**")
    if user_id in WHITELIST:
        return await Man.edit(
            "**Kau Gak Bisa Kontol Gban Dia Zenena Dia Adalah admin @Zenc0de 😡**"
        )
    if user_id:
        try:
...
async def gmute_user(client: Client, message: Message):
    ...
    if user.id in DEVS:
        return await Man.edit("**Gak Bisa Di Gmute Kontol Zenena dia Yang Buat Aku 🗿**")
    if user.id in WHITELIST:
        return await Man.edit(
            "**Kau Gak Bisa Kontol Gmute Dia Karena Dia Adalah admin @zennihh 😡**"
        )
    try:
