...
async def incomingpm(client: Client, message: Message):
    try:
        ...
        from PyroZen.helpers.SQL.pm_permit_sql import is_approved
...
async def auto_accept(client, message):
    try:
        from PyroZen.helpers.SQL.pm_permit_sql import approve, is_approved
    except BaseException:
        pass
    ...
    if message.chat.id in DEVS:
        try:
            ...
            await client.send_message(
                message.chat.id,
                f"<b>Menerima Pesan!!!</b>\n{message.from_user.mention} <b>Terdeteksi Developer PyroZen-Userbot</b>",
...
async def approvepm(client: Client, message: Message):
    try:
        from PyroZen.helpers.SQL.pm_permit_sql import approve
    except BaseException:
...
async def disapprovepm(client: Client, message: Message):
    try:
        from PyroZen.helpers.SQL.pm_permit_sql import dissprove
    except BaseException:
        await message.edit("Running on Non-SQL mode!")
        return
...
async def setpm_limit(client: Client, cust_msg: Message):
    ...
    try:
        from PyroZen.helpers.SQL.globals import addgvar
