...
from PyroZen.helpers.tools import get_arg, get_text, resize_media
from PyroZen.utils.tools import add_text_img, bash
...
async def kang(client: Client, message: Message):
    ...
    if replied and replied.media:
        if replied.photo:
            ...
            return
        media_ = await client.download_media(replied, file_name="PyroZen/resources/")
    else:
    ...
    if media_:
        ...
        while True:
            ...
            if exist.set.count >= limit:
                ...
                await Man.edit(
                    f"`Membuat Sticker Pack Baru {pack} Zenena Sticker Pack Sudah Penuh`"
                )
                continue
            break
...
async def tinying(client: Client, message: Message):
    ...
    im1 = Image.open("PyroZen/resources/blank.png")
    if ik.endswith(".tgs"):
