...
from PyroZen.helpers.constants import WWW
from PyroZen import app 
from PyroZen.helpers.PyroHelpers import SpeedConvert
from PyroZen.utils.tools import get_readable_time
from PyroZen.modules.bot.inline import get_readable_time
...
@Client.on_message(filters.command("Zen", cmd) & filters.me)
async def module_Zen(client: Client, message: Message):
    cdm = message.command
    help_arg = ""
    bot_username = (await app.get_me()).username
    if len(cdm) > 1:
        ...
        try:
            nice = await client.get_inline_bot_results(bot=bot_username, query="Zen")
            await asyncio.gather(
                client.send_inline_bot_result(
                    message.chat.id, nice.query_id, nice.results[0].id),
            )
